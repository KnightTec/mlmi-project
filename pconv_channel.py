...
class PConv2dChannel(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        self.mask_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, **conv_kwargs)
    ...
    def compute_masks(self, mask: Tensor4d) -> Tuple[Tensor4d, Tensor4d]:
        ...
        # Calculate sum(1) for each channel
        sum1_per_channel = self.kernel_size[0] * self.kernel_size[1] * self.in_channels
        mask_ratio = sum1_per_channel / (update_mask + 1e-8)
